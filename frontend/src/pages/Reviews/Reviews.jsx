...
import { businessService } from '../../api/apiService'; // NEW: Imported service
import './Reviews.css';
...
const Reviews = () => {
  ...
  // --- COMMENTED OUT: OLD JSON SERVER BASE ---
  ...
  // NEW: Fetch using businessService
  const fetchBusinessesByCategory = async () => {
    try {
      setLoading(true);
      // Calling your centralized apiService
      const response = await businessService.getByCategory(activeFilter);
      // apiClient handles the error throwing, so if we are here, it's a success
      setBusinesses(response.data || []);
    } catch (error) {
      console.error('Error fetching via businessService:', error.message);
      // Fallback to your local mock if DB is empty or fails
  ...
  /* --- COMMENTED OUT: OLD FETCH METHOD ---
  const fetchBusinessesByCategory = async () => {
    try {
      setLoading(true);
  ...
  */
  ...
  // NEW: Fetch featured using businessService
  const fetchFeaturedBusinesses = async () => {
    try {
      const response = await businessService.getFeatured();
  ...
  /* --- COMMENTED OUT: OLD FEATURED FETCH ---
  const fetchFeaturedBusinesses = async () => {
    try {
  ...
  */
  ...
  // Mock data fallback - KEPT FOR REFERENCE AS REQUESTED
      ...
      'on-campus': [
        {
          ...
          image: [
            { url: originalImages['on-campus'].mainImage, isPrimary: true },
            { url: originalImages['on-campus'].peekImage, isPrimary: false }
          ]
        },
        // ... (remaining mock data)
      ],
  ...
  // NEW: Search using businessService
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await businessService.search(searchQuery);
      setSearchResults(response.data || []);
    } catch (error) {
      console.error('Search error:', error.message);
      performLocalSearch(); // Fallback to local search if API fails
    } finally {
      setLoading(false);
    }
  };

  /* --- COMMENTED OUT: OLD SEARCH METHOD ---
  const handleSearch = async () => {
    try {
      setLoading(true);
      setIsSearching(true);
  ...
  */
  ...
  // Local search fallback - KEPT FOR REFERENCE
    ...
    return (
      <div className="main-card">
        <div 
          ...
          style={{ backgroundImage: `url('${mainImage?.url || originalImages[activeFilter].mainImage}')` }}
        ></div>
        <div className="main-card-content">
          ...
          {/* Using _id for MongoDB or id for Mock compatibility */}
          <Link to={`/customer-review/${business._id || business.id}`} state={{ business: business }}>
      ...
      <div className="peek-card">
        <Link to={`/customer-review/${business._id || business.id}`} state={{ business: business }}>
          <div 
            ...
            style={{ backgroundImage: `url('${peekImage?.url || originalImages[activeFilter].peekImage}')` }}
          ></div>
      ...
      <div className="hero-grid-container">
        ...
        {loading ? (
          ...
          businesses.map((business, index) => (
            <div key={business._id || business.id || index} className="hero-grid">
    ...
    <main>
      <div className="container">
        <div className="filter-bar">
          {['on-campus', 'delivery', 'off-campus'].map((type) => (
            <div 
              key={type}
